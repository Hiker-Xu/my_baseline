...
def main(opt):
    ...
    train_loader = DataLoader(
        dataset=SURREAL24(data_path=opt.data_dir),
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        err_best = min(err_best, err_test)
        if is_best:
...
def train(train_loader, model, criterion, optimizer,
          lr_init = None, lr_now = None, glob_step= None, lr_decay = None, 
...
def test(test_loader, model, criterion, stat_3d, procrustes=False):
    ...
    for i, (inps, tars) in enumerate(test_loader):
        ...
        # use procrustes to align output and target, in order to get minest distance
        if procrustes:
            for ba in range(inps.size(0)):
                gt = targets_unnorm[ba].reshape(-1, 3)
