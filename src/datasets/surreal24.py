...
class SURREAL24(Dataset):
    def __init__(self, data_path, is_train=True):
        ...
        # loading data
        train_2d_file = 'train_2d.npy'
        test_2d_file = 'test_2d.npy'

        if self.is_train:
            # load train data
            self.train_3d = np.load(os.path.join(data_path, 'train_3d.npy'))
            self.train_2d = np.load(os.path.join(data_path, train_2d_file))
            num_f = self.train_2d.shape[0]
            assert self.train_2d.shape[0] == self.train_3d.shape[0], '(training) 3d & 2d shape not matched' 
            self.train_inp = self.train_2d.reshape(num_f, 48)
            self.train_out = self.train_3d.reshape(num_f, 72)

        else:
            # load test data
            self.test_3d = np.load(os.path.join(data_path, 'test_3d.npy'))
            self.test_2d = np.load(os.path.join(data_path, test_2d_file))
            num_f = self.test_2d.shape[0]
            assert self.test_2d.shape[0] == self.test_3d.shape[0], '(testing) 3d & 2d shape not matched' 
            self.test_inp = self.test_2d.reshape(num_f, 48)
            self.test_out = self.test_3d.reshape(num_f, 72)
